...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  let rec lookup_class_h c_env = 
    match c_env with 
    | (id,m)::t ->
    if id = c_name
    then return m 
    else lookup_class_h t 
    | [] -> error "lookup_class: class c_name not found"
  in lookup_class_h c_env
    
let rec new_env_h : string list -> env ea_result  = fun fs ->
  match fs with
  | [] -> return EmptyEnv
  | h::t -> new_env_h t >>+ extend_env h (RefVal (Store.new_ref g_store (NumVal 0)))

let new_env : string list -> env ea_result = fun fs -> new_env_h (List.rev fs)
    ...
    sequence(List.map eval_expr es) >>= fun args ->
      (lookup_class c_name ! g_class_env) >>= fun (super, fields, methods) ->
        (new_env fields) >>= fun env ->
          ...
          | Some m -> apply_method "initialize" (ObjectVal(c_name, env)) args m >>=
            fun w -> return(ObjectVal(c_name, env)))
  | Send(e,m_name,es) ->
